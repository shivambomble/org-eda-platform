...
import { FileText, Activity, Trash2, RotateCcw } from "lucide-react";
...
const DatasetList = ({ datasets, projectId, refresh, canDelete }: any) => {
  ...
  const handleRestore = async (datasetId: string) => {
    if (!confirm("Are you sure you want to restore this dataset?")) {
      return;
    }
    try {
      const response = await axios.post(
        `${API_URL}/api/projects/${projectId}/datasets/${datasetId}/restore`,
        {},
        { headers: { Authorization: `Bearer ${getToken()}` } }
      );
  ...
  const filteredDatasets = isAdmin 
    ? datasets // Admin sees all datasets
    : datasets.filter((ds: any) => !ds.deleted_at); // Others see only active datasets
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-600">
            {filteredDatasets.map((ds: any) => {
              const isDeleted = !!ds.deleted_at;
              return (
                <tr key={ds.id} className={`hover:bg-slate-600/30 transition-colors ${isDeleted ? 'opacity-60' : ''}`}>
                  <td className="px-6 py-4 font-medium text-slate-200 flex items-center gap-2">
                    <FileText className="w-4 h-4 text-slate-400" />
                    {ds.file_name}
                    {isDeleted && <span className="text-xs text-red-400">(Deleted)</span>}
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-400">{(ds.size_bytes / 1024).toFixed(2)} KB</td>
                  <td className="px-6 py-4">
                    {isDeleted ? (
                      <span className="px-3 py-1 rounded-full text-xs font-bold inline-block bg-red-600/30 text-red-300 border border-red-500/50">
                        DELETED
                      </span>
                    ) : (
                      <span className={`px-3 py-1 rounded-full text-xs font-bold inline-block ${
                        ds.status === 'READY' ? 'bg-emerald-600/30 text-emerald-300 border border-emerald-500/50' : ''
                      }${ds.status === 'TRANSFORMED' || ds.status === 'EDA_COMPLETE' ? 'bg-purple-600/30 text-purple-300 border border-purple-500/50' : ''}
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      {isDeleted ? (
                        // Show restore button for deleted datasets (admin only)
                        isAdmin && (
                          <Button 
                            variant="secondary" 
                            size="sm" 
                            className="h-8 px-3 bg-green-600/30 text-green-300 hover:bg-green-600/50 border border-green-500/50"
                            onClick={() => handleRestore(ds.id)}
                          >
                            <RotateCcw className="w-3 h-3 mr-1" /> Restore
                          </Button>
                        )
                      ) : (
                        // Show normal actions for active datasets
                        <>
                          {ds.status === 'READY' && (
                            <Button 
                              variant="secondary" 
